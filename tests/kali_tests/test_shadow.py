...
Test Shadow from attacker perspective on Kali Linux.

These tests verify that Shadow runs correctly on Kali Linux
and that all core functionality works as expected.

Run with: python3 -m pytest tests/kali_tests/ -v
Run with: python3 tests/kali_tests/test_shadow.py
"""
...
class TestShadowKali(unittest.TestCase):
    """Test Shadow on Kali Linux"""
    ...
    def test_shadow_command_exists(self):
        """Test that shadow command exists"""
    ...
    def test_shadow_scan_no_sudo(self):
        """Test shadow scan without sudo (should work for non-root checks)"""
        result = subprocess.run(
            ['shadow', '--scan'],
            capture_output=True, text=True,
            timeout=120
        )
        # May still work if not root (some checks don't need root)
        # Just check it doesn't crash
        self.assertLess(result.returncode, 2, "shadow --scan crashed")
    ...
    def test_shadow_report(self):
        ...
        # Report may still exit with 0 even if no data
    ...
    def test_shadow_config_exists(self):
        ...
        # Config may not exist on fresh install, that's OK
...
class TestShadowKaliNegative(unittest.TestCase):
    """Negative tests for Shadow on Kali Linux"""
...
class TestShadowModuleImports(unittest.TestCase):
    """Test that all Shadow modules can be imported"""
